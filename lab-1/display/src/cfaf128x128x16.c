...
// display with an ST7735S.  This version uses an SSI interface to the display
// controller. This driver was made to be used with the TivaWare grlib driver
// set (ti.com/tivaware), and was based on the Kentec320x240x16_ssd2119_spi
// driver for the ek-tm4c1294xl board, the cfal96x64x16 driver for the
// dk-tm4c123g board and the Energia's Screen_HX8353E driver for dk-tm4c1294xl
// board.
//
// All rights reserved. 
...
#define delay(d) SysCtlDelay(d*10)
...
#define DISPLAY_SSI_CLOCK           12000000 // 12MHz
...
cfaf128x128x16SsiTransfer(uint8_t command8){
	uint32_t received32;
	SSIDataPut(DISPLAY_SSI_BASE, command8);
	while(SSIBusy(DISPLAY_SSI_BASE));
	SSIDataGet(DISPLAY_SSI_BASE, &received32);
...
cfaf128x128x16WriteCommand(uint8_t command8){		
		...
		GPIOPinWrite(DISPLAY_CS_PORT, DISPLAY_CS_PIN, 0);
		cfaf128x128x16SsiTransfer(command8);
		GPIOPinWrite(DISPLAY_CS_PORT, DISPLAY_CS_PIN, DISPLAY_CS_PIN);
}
...
//*****************************************************************************
static void
cfaf128x128x16PixelDrawMultiple(void *pvDisplayData, int32_t i32X, int32_t i32Y,
...
InitSSICom(void){
	...
	g_ui32SysClock = SystemCoreClock; // __SysCtlClockGet();
	...
	GPIOPinConfigure(DISPLAY_PINCFG_SSIRX);
	
	GPIOPinTypeSSI(DISPLAY_SSI_PORT, DISPLAY_SSI_PINS);
	
	SSIClockSourceSet(DISPLAY_SSI_BASE, SSI_CLOCK_PIOSC);
	SSIConfigSetExpClk(DISPLAY_SSI_BASE, g_ui32SysClock, 
		SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, DISPLAY_SSI_CLOCK, 8);

	SSIEnable(DISPLAY_SSI_BASE);
	while(SSIDataGetNonBlocking(DISPLAY_SSI_BASE, &initialData));
}
...
cfaf128x128x16Init(void){	
	...
	SysCtlPeripheralEnable(DISPLAY_C_D_GPIO_PERIPH);
	SysCtlPeripheralEnable(DISPLAY_CS_GPIO_PERIPH);
